...
def parse_args(input_args=sys.argv):
    ...
    parser.add_argument("--log-folder", type=str, default="/tmp")
    ...
    parser.add_argument("--wandb-project-name", type=str, default="trafficgen")
    ...
    parser.add_argument("--env", type=str, default="trafficgen")
    ...
    parser.add_argument(
        "--data_path", type=str, default="/cmlscratch/anirudhs/trafficgen/data"
    )
    parser.add_argument(
        "--results_path", type=str, default="/cmlscratch/anirudhs/trafficgen/results"
    )
    parser.add_argument(
        "--model_name", type=str, default="Qwen/Qwen2.5-7B-Instruct-AWQ"
    )
    parser.add_argument("--env_type", type=str, default="car", choices=["car", "flow"])
    parser.add_argument(
        "--run_type", type=str, default="random", choices=["llm", "random"]
    )
    parser.add_argument(
        "--curriculum",
        type=str,
        ...
        choices=[
            ...
            "NoneCurriculum",
            "ACCELCurriculum",
            "LLMCurriculumWithExploration",
            "LLMCurriculumWithDiversity",
        ],
    )
    ...
    # if thread number not set, use 3/4 cpus
    if args.threads == -1:
        cpu_count = 3 * multiprocessing.cpu_count() // 4
        if cpu_count < 1:
            cpu_count = 1
