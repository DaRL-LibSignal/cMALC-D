...
class Logger:
    ...
    def init(self, kwargs):
        args = kwargs.copy()
        self.log_folder = kwargs["work_folder"] + "/" + kwargs["log_folder"] + "/"
        ...
        gitdata = "unknown"
        try:
            gitdata = os.popen("git log --pretty=oneline -n 1").readlines()[0].strip()
        except Exception:
            pass
        ...
        if kwargs["cityflow_config"] != "":
            ...
            with open(inf + "flow.json", "w") as f:
                f.write(open(kwargs["work_folder"] + cf_config["FLOW_FILE"]).read())
            with open(inf + "roadnet.json", "w") as f:
                f.write(open(kwargs["work_folder"] + cf_config["ROADNET_FILE"]).read())
