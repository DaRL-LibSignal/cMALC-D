...
class TrafficSignal:
    ...
    def __init__(self, ID, eng, yellow_time, roadnet_info):
        ...
        self.set_eng_phase()
        self.counts = [[0] * 2] * len(self.roadnet_info["roadlinks"])
        self.wait_counts = [[0] * 2] * len(self.roadnet_info["roadlinks"])
    ...
    def _set_observation_space(self):
        ...
        self.observation_space = {
            ...
            "TSphase": np.array(self.roadnet_info["phases"][1:], dtype=object),
            "TStime": np.array([1]),
            "Envtime": np.array([1]),
    ...
    def get_state(self):
        ...
        for roadlink in range(len(self.roadnet_info["roadlinks"])):
            count = self.counts[roadlink]
            wait_count = self.wait_counts[roadlink]
...
class Intersection:
    ...
    def _get_average_vehicle_count(self, type, weight):
        res = 0
        cres = 0
        counts, wait_counts = self.TS.counts, self.TS.wait_counts
        if type.lower() == "flow":
            values = counts
        ...
        for roadlink in range(len(self.roadnet_info["roadlinks"])):
            count = values[roadlink]
            if count[1] == 0:
                pass
            else:
                res += count[0] / count[1]
                cres += 1
        return res / cres * weight if cres != 0 else 0
...
class CityFlowEnv(gym.Env):
    def __init__(
        ...
        self.date_time_str = time.strftime("%Y_%m_%d_%H_%M_%S")
        self.folder_name = "%s_%s_%d_%d" % (
            self.date_time_str,
            self.curriculum_name,
            seed,
            test_index,
        )
        self.env_name = os.path.basename(self.config["CONFIG_FILE"]).split(".")[0]
        self.log_path = os.path.join(self.log_path, self.folder_name, self.env_name)
        os.makedirs(self.log_path, exist_ok=True)
        if self.curriculum_name != "NoneCurriculum":
            self.generate_task()
        ...
        config_dict = {
            ...
            "seed": seed,
            "dir": "",
            "roadnetFile": self.config["ROADNET_FILE"],
            "flowFile": self.config["FLOW_FILE"],
            "rlTrafficLight": True,
            "laneChange": True,
        ...
        self.num_lanes_per_ts = len(
            self.list_intersection[0].TS.roadnet_info["roadlinks"]
        )
        ...
        self._set_observation_space()
        self._set_action_space()
        self.update_lane_vehicle_counts()

    def generate_task(self):
        if self.env_type == "car":
            flow_file = os.path.join(self.work_folder, self.config["FLOW_FILE"])
            flow_json = copy.deepcopy(json.load(open(flow_file, "r")))
            for key in self.task_params:
                updated_flow_json = update_json_key(
                    flow_json, key, self.task_params[key]
                )
            with open(f"{self.log_path}/flow.json", "w") as f:
                json.dump(updated_flow_json, f)
            self.config["FLOW_FILE"] = f"{self.log_path}/flow.json"
            self.config["ROADNET_FILE"] = os.path.join(
                self.work_folder, self.config["ROADNET_FILE"]
            )
        elif self.env_type == "flow":
            flow_file = os.path.join(self.work_folder, self.config["FLOW_FILE"])
            flow_json = copy.deepcopy(json.load(open(flow_file, "r")))
            walk_sizes = [len(i["route"]) for i in flow_json]
            graph, virtual_nodes, edge_name_map = build_traffic_graph(self.config)
            intersections = list(self.config["ROADNET_INFO"].keys())
            walks = generate_flow(
                graph,
                {
                    intersections[i]: self.task_params[i]
            ...
            self.config["FLOW_FILE"] = os.path.join(
                self.work_folder, self.config["FLOW_FILE"]
            )
            self.config["ROADNET_FILE"] = os.path.join(
                self.work_folder, self.config["ROADNET_FILE"]
            )
    ...
    def init_engine(self):
        time.sleep(1)
        self.eng = Engine(self.config_path, thread_num=1)

    def update_lane_vehicle_counts(self):
        lane_counts = self.eng.get_lane_vehicle_count()
        if "road_0_4_0_1" not in lane_counts or "road_4_0_1_2" not in lane_counts:
            pass
        lane_wait_counts = self.eng.get_lane_waiting_vehicle_count()
        for inter in self.list_intersection:
            counts = []
            wait_counts = []

            for roadlink in inter.TS.roadnet_info["roadlinks"]:
                lanelinks = roadlink["lanelinks"]
                in_count = sum(lane_counts[start] for start, _ in lanelinks)
                in_wait_count = sum(lane_wait_counts[start] for start, _ in lanelinks)

                total_in = in_count + in_wait_count

                counts.append([in_count, total_in])
                wait_counts.append([in_wait_count, total_in])

            inter.TS.counts = counts
            inter.TS.wait_counts = wait_counts
    ...
    def step(self, action):
        ...
        # success_rate = max(0, 1 - average_delay / average_time)
