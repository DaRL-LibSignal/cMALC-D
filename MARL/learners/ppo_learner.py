...
from MARL.modules.critics import REGISTRY as critic_resigtry
...
class PPOLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for key in critic_train_stats:
            log_stats[key] = sum(critic_train_stats[key]) / ts_logged

        log_stats["advantage_mean"] = (
            advantages * mask
    ...
    def train_critic_sequential(self, critic, target_critic, batch, rewards, mask):
        ...
        running_log = {
            ...
            "q_taken_mean": [],
            "value_estimate": [],
            "positive_value_loss": [],  # Add this for positive value loss
        }
        ...
        # Calculate positive value loss (PLR)
        positive_value_loss = th.max(td_error, th.zeros_like(td_error))

        positive_value_loss = (positive_value_loss * mask).sum() / mask.sum()
        ...
        running_log["value_estimate"].append(v.mean().item())
        running_log["positive_value_loss"].append(
            positive_value_loss.item()
        )  # Log positive value loss

        return masked_td_error, running_log

    def compute_gae(self, rewards, values, mask, gamma=0.99, gae_lambda=0.95):
        """
        Compute Generalized Advantage Estimation (GAE)
        """
        batch_size, max_seq_length, n_agents = rewards.shape

        # Add dummy 0 at end for easier calculation
        values_plus = th.zeros(
            batch_size, max_seq_length + 1, n_agents, device=values.device
        )
        values_plus[:, :-1] = values
        rewards_plus = th.zeros(
            batch_size, max_seq_length + 1, n_agents, device=rewards.device
        )
        rewards_plus[:, :-1] = rewards

        deltas = (
            rewards_plus[:, :-1]
            + gamma * values_plus[:, 1:] * mask
            - values_plus[:, :-1]
        )

        advantages = th.zeros_like(deltas)
        advantage = th.zeros(batch_size, n_agents, device=values.device)

        for t in reversed(range(max_seq_length)):
            advantage = deltas[:, t] + gamma * gae_lambda * mask[:, t] * advantage
